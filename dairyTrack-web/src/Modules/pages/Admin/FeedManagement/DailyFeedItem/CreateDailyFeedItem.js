  ...
  useEffect(() => {
    const fetchFeedsAndStocks = async () => {
      try {
        ...
        const feedsWithStock = feedResponse.feeds.map((feed) => {
          const stockData = stockResponse.feeds.find((s) => String(s.id) === String(feed.id));
          console.log(`Matching Feed ID: ${feed.id}, Stock Data:`, stockData);

          let stockValue = 0;
          let unitValue = feed.unit || "kg";

          if (stockData && stockData.FeedStock) {
            stockValue = parseFloat(stockData.FeedStock.stock) || 0;
            unitValue = stockData.FeedStock.unit || unitValue;
          } else {
          ...
          return {
            id: feed.id,
            name: feed.name || (feed.FeedType?.name || `Feed #${feed.id}`),
            stock: stockValue,
            unit: unitValue,
          };
        });
        ...
        if (feedsWithStock.length === 0) {
          setError("Tidak ada pakan tersedia.");
        }
      } catch (err) {
